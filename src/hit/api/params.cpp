...
namespace hit {
    ...
    BootstrappingParams::BootstrappingParams(uint64_t secret_hamming_weight, uint64_t approx_range,
                                             ...
                                             vector<uint8_t> CtSLevels, vector<uint8_t> StCLevels, double maxN1N2Ratio)
    ...
    CKKSParams::CKKSParams(latticpp::Parameters params) : params(move(params)) {
    ...
    CKKSParams::CKKSParams(int num_slots, int log_scale, int max_ct_level, int num_ks_primes, 
        ...
        params = latticpp::newParametersFromLogModuli(
            log2(num_slots) + 1, 
            log_qi, 
            max_ct_level + 1, 
            log_pi, 
    ...
    int CKKSParams::num_slots() const {
        return latticpp::numSlots(params);
    }

    int CKKSParams::log_scale() const {
        return static_cast<int>(log2(latticpp::scale(params)));
    }

    int CKKSParams::max_ct_level() const {
        return latticpp::maxLevel(params);
    }
