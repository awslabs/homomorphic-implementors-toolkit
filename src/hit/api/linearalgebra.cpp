...
namespace hit {
    ...
    bool EncryptedMatrix::initialized() const {
        return unit.initialized()  && !cts.empty() && num_vertical_units() == cts.size() && !cts[0].empty() && num_horizontal_units() == cts[0].size();
    }
    ...
    Matrix LinearAlgebra::decrypt(const EncryptedMatrix &mat) const {
        ...
        for(int i = 0; i < mat.cts.size(); i++) {
            ...
            for(int j = 0; j < mat.cts[0].size(); j++) {
                row_pieces[j] = Matrix(mat.encoding_unit().encoding_height(), mat.encoding_unit().encoding_width(), inst.decrypt(mat.cts[i][j]));
            }
    ...
    Vector LinearAlgebra::decrypt(const EncryptedRowVector &vec) const {
        ...
        for(int i = 0; i < vec.cts.size(); i++) {
            mat_pieces[i] = Matrix(vec.encoding_unit().encoding_height(), vec.encoding_unit().encoding_width(), inst.decrypt(vec.cts[i]));
        }
     ...
     */
    EncryptedColVector LinearAlgebra::matrix_matrix_mul_loop(const EncryptedMatrix &matrix_aTrans, const EncryptedMatrix &matrix_b, const double scalar, int k, bool transpose_unit) {
        ...
        int num_slots = matrix_aTrans.encoding_unit().encoding_width()*matrix_aTrans.encoding_unit().encoding_height();
    ...
    vector<EncryptedColVector> LinearAlgebra::multiply_common(const EncryptedMatrix &matrix_aTrans, const EncryptedMatrix &matrix_b, double scalar, bool transpose_unit) {
...
# else // !HIT_USE_TBB
    ...
    EncryptedMatrix LinearAlgebra::multiply(const EncryptedMatrix &matrix_aTrans, const EncryptedMatrix &matrix_b, double scalar) {
        if (!matrix_aTrans.initialized() || !matrix_b.initialized() || matrix_aTrans.height() != matrix_b.height() || matrix_aTrans.encoding_unit() != matrix_b.encoding_unit()) {
        ...
        int result_vertical_units = ceil(matrix_aTrans.width() / static_cast<double>(matrix_aTrans.encoding_unit().encoding_height()));
    ...
    EncryptedMatrix LinearAlgebra::multiply_unit_transpose(const EncryptedMatrix &matrix_aTrans, const EncryptedMatrix &matrix_b, double scalar) {
         ...
         */
        if (!matrix_aTrans.initialized() || !matrix_b.initialized() || matrix_aTrans.height() != matrix_b.height() || matrix_aTrans.encoding_unit() != matrix_b.encoding_unit() ||
            matrix_b.height() > matrix_b.encoding_unit().encoding_height() || matrix_b.encoding_unit().encoding_width() > matrix_b.encoding_unit().encoding_height() ||
            matrix_aTrans.width() > matrix_b.encoding_unit().encoding_width() || matrix_b.width() > matrix_b.encoding_unit().encoding_width()) {
            throw invalid_argument("Arguments to LinearAlgebra::multiply_unit_transpose do not have compatible dimensions.");
        }
        ...
        EncodingUnit tranposeUnit = EncodingUnit(matrix_b.encoding_unit().encoding_width(), matrix_b.encoding_unit().encoding_height());
