...
class CKKSInstance {
   public:
     ...
     * of security.
     */
    static CKKSInstance* getNewPlaintextInstance(int numSlots, bool verbose = false, bool useSEALParams = true);
     ...
     */
    static CKKSInstance* getNewScaleEstimatorInstance(int numSlots, int multDepth, bool verbose = false,
     ...
     * See `getNewPlaintextInstance` for description of `useSEALParams`.
     */
    static CKKSInstance* getNewHomomorphicInstance(int numSlots, int multDepth, int logScale, bool verbose = false,
                                                   bool useSEALParams = true,
                                                   const std::vector<int>& galois_steps = std::vector<int>());

    static CKKSInstance* loadHomomorphicInstance(std::istream& paramsStream, std::istream& galoisKeyStream,
     ...
     * See `getNewPlaintextInstance` for description of `useSEALParams`.
     */
    static CKKSInstance* getNewDebugInstance(int numSlots, int multDepth, int logScale, bool verbose = false,
                                             bool useSEALParams = true,
                                             const std::vector<int>& galois_steps = std::vector<int>());

    /* Create a new debug instance from the provided parameters and keys */
    static CKKSInstance* loadDebugInstance(std::istream& paramsStream, std::istream& galoisKeyStream,
                                           std::istream& relinKeyStream, std::istream& secretKeyStream,
                                           bool verbose = false);

    /* For evaluation only. Decryption is not available. */
    static CKKSInstance* loadEvalInstance(std::istream& paramsStream, std::istream& galoisKeyStream,
                                          std::istream& relinKeyStream, bool verbose = false);

    /* For encryption and decryption only. Evaluation is not available. */
    static CKKSInstance* loadNonEvalInstance(std::istream& paramsStream, std::istream& secretKeyStream,
                                             bool verbose = false);
     ...
     */
    void encryptColVec(const std::vector<double>& plain, int matHeight, CKKSCiphertext& destination, int level = -1);
     ...
     */
    void encryptRowVec(const std::vector<double>& plain, int matWidth, CKKSCiphertext& destination, int level = -1);
...
// evaluator, while `DEBUG` loads a debug evaluator. `NONEVALUATION` is useful for cliend-side computation which don't
// need to perform any evaluation. A `NONEVALUATION` instance can *ONLY* be used for encryption and decryption.
CKKSInstance* tryLoadInstance(int numSlots, int multDepth, int logScale, Mode mode,
                              const std::vector<int>& galois_steps = std::vector<int>());
