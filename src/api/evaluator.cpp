...
<<<<<<< HEAD
    ...
    CKKSCiphertext CKKSEvaluator::negate_inplace(CKKSCiphertext &ct) {
        ct = negate(ct);
    }
    ...
    void CKKSEvaluator::mod_down_to_internal(CKKSCiphertext &ct, const CKKSCiphertext &target) {
        ct = mod_down_to(ct, target);
    }

    void CKKSEvaluator::mod_down_to_min(CKKSCiphertext &ct1, CKKSCiphertext &ct2) {
        VERBOSE(cout << "Equalizing HE levels" << endl);
        mod_down_to_min_internal(ct1, ct2);
    }
    ...
    CKKSCiphertext CKKSEvaluator::rescale_to_next(const CKKSCiphertext &ct) {
        ...
        rescale_to_next_internal(ct);
    }

    CKKSCiphertext CKKSEvaluator::rescale_to_next_inplace(CKKSCiphertext &ct) {
