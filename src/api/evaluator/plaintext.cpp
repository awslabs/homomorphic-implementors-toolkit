...
void PlaintextEval::print_stats(const CKKSCiphertext &ct) const { // NOLINT(readability-convert-member-functions-to-static)
  ...
  cout << "    + Exact plaintext logmax: " << log2(exactPlaintextMaxVal) << " bits (scaled: " <<
          log2(ct.scale)+log2(exactPlaintextMaxVal) << " bits)" << endl;
...
CKKSCiphertext PlaintextEval::multiply_plain_mat_internal(const CKKSCiphertext &ct, const vector<double> &plain) {
  CKKSCiphertext dest = ct;
  Plaintext temp;
