...
CKKSInstance *CKKSInstance::loadDebugInstance(istream &paramsStream, istream &galoisKeyStream, istream &relinKeyStream,
                                              istream &secretKeyStream, bool verbose) {
    return new CKKSInstance(paramsStream, &galoisKeyStream, &relinKeyStream, &secretKeyStream, verbose, DEBUG);
}
CKKSInstance *CKKSInstance::loadEvalInstance(istream &paramsStream, istream &galoisKeyStream, istream &relinKeyStream,
                                             bool verbose) {
...
void CKKSInstance::sharedParamInit(int numSlots, int multDepth, int logScaleIn, bool useSEALParams, bool verbose) {
    ...
    if (logScale < MIN_LOG_SCALE) {
        ...
        buffer << "Invalid parameters: Implied logScale is " << logScale << ", which is less than the minimum, "
               << MIN_LOG_SCALE << ". Either increase the number of slots or decrease the number of primes." << endl;
    ...
    if (useSEALParams) {
        ...
        securityWarningBox(
            "YOU ARE NOT USING SEAL PARAMETERS. Encryption parameters may not achieve 128-bit security. DO NOT USE IN "
            "PRODUCTION.");
